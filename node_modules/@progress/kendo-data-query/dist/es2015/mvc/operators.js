...
const typedFormatter = encode => runOrEmpty(hasField, either(isStringValue, compose(filterFormatter, quote, encode ? encodeValue : identity), dateFormatter));
const join = ({ logic }) => `~${logic}~`;
const serialize = encode => serializeFilters(filter => either(isCompositeFilterDescriptor, serialize(encode), typedFormatter(encode))(filter), join);
