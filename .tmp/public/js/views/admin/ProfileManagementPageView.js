...
admin.ProfileManagementPageView = Backbone.View.extend({
	...
	render: function() {					
        ...
        this.subscriptionList = new admin.ProfileSubscriptionListView({el: $( '#subscriptionList' )});
  ...
  dataModalOpen : function(e){
	...
	passwordChange:function(){
		...
		if((password.length <= 10 && password.length >= 6) && (confirmPassword.length <= 10 && confirmPassword.length >= 6)) {
			...
			// $( "div.success" ).html("Password Changed Successfully.");
	    ...
	    if(file){
	     	$.ajax({
	          ...
	          success: function(data) {
	                ...
	                $('body').find('#esp-user-profile img').attr('src' , data.imagePath);
