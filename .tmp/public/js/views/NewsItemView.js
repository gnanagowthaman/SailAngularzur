...
app.NewsItemView = Backbone.View.extend({
	...
	arrowClicked: function(){
		...
		if(this.$el.find('#arrow').parent().hasClass('opened')){
			...
			this.$el.find('#arrow').parent().find('.news-db-text').height(38);
			this.$el.find('#arrow').parent().removeClass('opened');
		} else {
			var height = this.$el.find('#arrow').parent().find('.news-db-text').height();
			var scrollHeight = this.$el.find('#arrow').parent().find('.news-db-text')[0].scrollHeight;
			var diff = scrollHeight-height;

			this.$el.find('#arrow').parent().parent().height(95+diff);
			this.$el.find('#arrow').parent().find('.news-db-text').height(scrollHeight);
