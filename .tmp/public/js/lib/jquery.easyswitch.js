	...
	$.fn.easyswitch = function(options) {
		...
		this.each(function(i, item) {
			...
			html = '<span class="'+opts['class-label']+'">'+opts['label-on']+'</span>\
					...
					<span id="easyswitch-slider" class="'+opts['class-slider']+'"></span>';
			if (opts['name']) {
			...
			self.append(html).click(function() {
				if (self.hasClass(opts['class-off'])) {
					self.find('.'+opts['class-slider']).animate({left:'50%'}, 'fast', function() {
						...
						// opts['callback'](value, self); [original code]
						opts['callback'].call(opts['context1'], value); //this is added by Vahai
					});
				} else {
					self.find('.'+opts['class-slider']).animate({left:'2px'}, 'fast', function() {
						self.removeClass(opts['class-on']).addClass(opts['class-off']);
						var value = 0;
						$('#easyswitch-'+opts['name']).val(value);
						// opts['callback'](value, self);
						opts['callback'].call(opts['context1'], value);
					});
				}
			});
			...
			if (opts['default']) {
				...
				self.find('.'+opts['class-slider']).css({left:'2px'});
			}
		});
	}
})(jQuery);