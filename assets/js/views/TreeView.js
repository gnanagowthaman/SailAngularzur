...
app.TreeView = Backbone.View.extend({
  ...
  events: {
    ...
    'click .showgeomap' : 'showGeoMap',
    'click .showdomainlogo' : 'showDomainLogo',
     'click #regesearch' : 'showSearch'
...
//    'change  #treeselect' : 'selectdoc',
  ...
  initTree : function()
  {
    ...
    if(this.mode == 0){
      console.log("checking mode :: ", this.mode);
    ...
    //Define Map of Geos & Domains
    this.geoMap = {"European Union" : "europe", 
                  "United States"  : "usaandcanada",  
                  "Asia Pacific"   : "apac",
                  "Middle East"    : "middleeast",    
                  "All geography"  : "all"};
    this.domainMap =  {"Banking & Credit Union" : "bankingandcreditunions", 
                  "Payment Services" : "paymentservices",       
                  "Financial Markets" : "financialmarkets",      
                  "Financial Crime" : "financialcrime",        
                  "Insurance" : "insurance",              
                  "Pension" : "pensions",                
                  "Supervisory" : "supervisory",
                  "Consumer Finance" : "consumerfinance"};
    this.geographyName = "European Union";
   ...
   //   this.coldDocList.fetch(),
      ...
      switch(_self.mode) {                        
        case '1':
         // window.history.back();
          console.log('openDoc');
        //  $('.regulation-tracker').find('.panel-title .rt-db-label').append('<button id="regesearch" type="button" >back to search</button>');
        $('#regsearch').show();
	  _self.openDoc(); 
          _self.reset();
          break;
 ...
 //   var subdocs = new app.MSubDocumentList();
  ...
  openDoc: function() {
    ...
    data.spl = parseInt(app.ClientAppRouter.spl, 10);
    data.fid = parseInt(app.ClientAppRouter.fid, 10);

    console.log("dtdta0",data);
    if (this.filterDoctype()) {
      if (data.spl == 1) {
        this.renderSingleSpDoc(data);
      } else {
  ...
  renderSingleSpDoc: function(data) {
    console.log(data);
    var _self = this;
      ...
      success: function(data) {
        ...
        if (data.hasOwnProperty('path')) {
          ...
          if ( mat == ".pdf") {
            ...
            pdf.render({ path : path, "selectedDocEl": _self.selectedDocEl});
          } else {
            $("#canvas").empty();
            $("#canvas").unbind();              
            ...
            xls.render({ path : path, "selectedDocEl": _self.selectedDocEl });
          }                    
        } else {
            ...
            _self.selectedDocEl.addClass('docclick');
          }  
      },
      error: function(data) {
        try{
  ...
  if (  data.spl && data.spl == 1 ) {
        ...
        console.log(app.ClientAppRouter.spldocname); 
          ...
          $('.widget').prepend("<div id='regtitle' style='text-align:center;'><font size='5'>" + selectedDoc + "</font></div>");          
            ...
            console.log("render spl", this.selectedDocEl);
            xlss.render_spl({ data : data, "selectedDocEl": this.selectedDocEl });
  ...
  domSelected : function(e) { 
    ...
    this.selectedDocEl = $(e.target); //will be used in xlsView & pdfView to enable click event
    // this.selectedDocEl = e.target; 
    console.log('this.selectedDocEl: ', this.selectedDocEl);
  ...
  showGeoMap : function(e) {
    console.log('SHOWING MAP TRIGGERED');
    var gname = $(e.target).text();
    this.geographyName = gname;
    console.log('GEO ', gname);
    switch (gname) {
      case 'European Union':
        $('#show-geo-map').attr('class', 'guide-tree europe');
        break;
      case 'United States':
        $('#show-geo-map').attr('class', 'guide-tree usaandcanada');
        break;
      case 'Asia Pacific':
        $('#show-geo-map').attr('class', 'guide-tree apac');
        break;
      case 'Middle East':
        $('#show-geo-map').attr('class', 'guide-tree middleeast');
        break;                
      default:
        $('#show-geo-map').attr('class', 'guide-tree europe'); 
    };
    //remove domain log & text
    $('#show-domain-logo').attr('class', 'logo-selection-tree');
    $('#show-domain-logo').empty();
  ...
  //    var gflag = item.attributes.name.toUpperCase() == "ALL";
      ...
      delem = elem.clone();
      clelem = lelem.clone();
      var spanEl = span.clone();
      spanEl.addClass('showgeomap');
      var gid = item.get('id');
      if (currentgid == gid) {
      ...
      // delem.text( item.get('name')); //Commented By Niranjan to show geo Map at right side 

      spanEl.attr('gid', item.get('id'));
      spanEl.text( item.get('name'));
  ...
  renderDomain : function(delem,sid, cntid, gid) {
    ...
    col4.each(function ( item) 
    {
      ...
      relem.attr('sid', sid); 
       relem.attr('did', did); 
      var dname = item.get('name');   
      // relem.text( dname); //Comment By Niranjan to show domain logo & Txt
      //Below span code (5 lines) added by Niranjan
      var spanEl = span.clone();
      spanEl.addClass('showdomainlogo');      
      spanEl.attr('gid', item.get('gid'));
      spanEl.text( item.get('name'));       
      relem.append(spanEl);
  ...
  showDomainLogo : function(e) {
    console.log('SHOWING DOMAIN TRIGGERED');
    var dname = $(e.target).text();
    ...
    var _self = this;
     ...
     var gid = $(e.target).attr('gid');
      ...
      if(gid == 2){
        $('#show-geo-map').attr('class', 'guide-tree europe');
      _self.geographyName = "European Union";
      }else if(gid == 3){
        $('#show-geo-map').attr('class', 'guide-tree usaandcanada');
      _self.geographyName = "United States";
     }else if(gid == 4){
      $('#show-geo-map').attr('class', 'guide-tree apac');
      _self.geographyName ="Asia Pacific";
     }else if(gid == 5){
      $('#show-geo-map').attr('class', 'guide-tree middleeast');
      _self.geographyName = "Middle East";
     }else {
      $('#show-geo-map').attr('class', 'guide-tree europe'); 
           _self.geographyName="All Geography";
      }
    ...
    switch (dname) {
      case 'Banking & Credit Union':
        $('#show-domain-logo').attr('class', 'logo-selection-tree bankingandcreditunions');
        $('#show-domain-logo').html('<div>' + dname.toUpperCase() + ' <br/>IN THE ' + _self.geographyName.toUpperCase() + '</div>');
        break;
      case 'Payment Services':
        $('#show-domain-logo').attr('class', 'logo-selection-tree paymentservices');
        $('#show-domain-logo').html('<div>' + dname.toUpperCase() + ' <br/>IN THE ' + _self.geographyName.toUpperCase() + '</div>');
        break;
      case 'Financial Markets':
        $('#show-domain-logo').attr('class', 'logo-selection-tree financialmarkets');
        $('#show-domain-logo').html('<div>' + dname.toUpperCase() + ' <br/>IN THE ' + _self.geographyName.toUpperCase() + '</div>');
        break;
      case 'Financial Crime':
        $('#show-domain-logo').attr('class', 'logo-selection-tree financialcrime');
        $('#show-domain-logo').html('<div>' + dname.toUpperCase() + ' <br/>IN THE ' + _self.geographyName.toUpperCase() + '</div>');
        break;
      case 'Insurance':
        $('#show-domain-logo').attr('class', 'logo-selection-tree insurance');
        $('#show-domain-logo').html('<div>' + dname.toUpperCase() + ' <br/>IN THE ' + _self.geographyName.toUpperCase() + '</div>');
        break;
      case 'Pension':
        $('#show-domain-logo').attr('class', 'logo-selection-tree pensions');
        $('#show-domain-logo').html('<div>' + dname.toUpperCase() + ' <br/>IN THE ' + _self.geographyName.toUpperCase() + '</div>');
        break;
      case 'Supervisory':
        $('#show-domain-logo').attr('class', 'logo-selection-tree supervisory');
        $('#show-domain-logo').html('<div>' + dname.toUpperCase() + ' <br/>IN THE ' + _self.geographyName.toUpperCase() + '</div>');
        break;
      case 'Consumer Finance':
        $('#show-domain-logo').attr('class', 'logo-selection-tree consumerfinance');
        $('#show-domain-logo').html('<div>' + dname.toUpperCase() + ' <br/>IN THE ' + _self.geographyName.toUpperCase() + '</div>');
        break;                        
      default:
        $('#show-domain-logo').attr('class', 'logo-selection-tree bankingandcreditunions');
...
renderTreeDomain:function(){
   var selectedGeo=app.ClientAppRouter.gname;
    var dname=app.ClientAppRouter.dname;
      ...
      if (selectedGeo) {
        ...
        if( i == 2){
            ...
            $(liTag).children('ul').children('li').each(function() { //each items is country        
             ...
             i = i+1;
             $(liTag).children('ul').children('li').each(function() { //each items is state
                console.log('UL STATE TAG: ', $(this)); 
                $(this).children('ul').children('li').each(function() { //each items is domain
                  var domainName = $(this).children('a').text();
                  var domainli = $(this);
 ...
 if(selectedGeo == "European Union"){
     ...
     switch (dname) {
      ...
      }else if(selectedGeo == "United States"){
        $('#show-geo-map').attr('class', 'guide-tree usaandcanada');
      _self.geographyName = "United States";
     ...
     }else if(selectedGeo == "Asia Pacific"){
      $('#show-geo-map').attr('class', 'guide-tree apac');
      _self.geographyName ="Asia Pacific";
     ...
     }else if(selectedGeo == "Middle East"){
      $('#show-geo-map').attr('class', 'guide-tree middleeast');
      _self.geographyName = "Middle East";
     ...
     switch (dname) {
      ...
      case 'Banking & Credit Union':
        $('#show-domain-logo').attr('class', 'logo-selection-tree bankingandcreditunions');
        $('#show-domain-logo').html('<div>' + dname.toUpperCase() +  '</div>');
        break;
      case 'Payment Services':
        $('#show-domain-logo').attr('class', 'logo-selection-tree paymentservices');
        $('#show-domain-logo').html('<div>' + dname.toUpperCase() + '</div>');
